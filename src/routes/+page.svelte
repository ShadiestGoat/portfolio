<script lang="ts">
	...
	import type { Langs, LangShares, Share } from '$lib/pie/draw';
	...
	import { blurTag, focusTag, selectTag } from '$lib/pie';
	...
	function pie(l: LangShares): Share[] {
		const items = Object.keys(l)
			.sort()
			.filter((k) => (colors[k] && l[k] >= 0.07 ? true : false));
		...
		const parsed: Share[] = items.map((k) => ({
			title: k,
			share: l[k] / divTotal,
			bg: colors[k][0],
			fg: colors[k][1]
	...
	function getLangs(l: Langs) {
		...
		const newLangs = allLangs.filter(
			(v) => colors[v] && v != 'Others' && (langs.backend[v] >= 0.07 || langs.frontend[v] >= 0.07)
		);
	...
	let selectedTag = writable<string>('');
	let tagWasClicked = writable<boolean>(false);
</script>
	...
	<div class="col">
		...
		<div class="row" id="pie-row">
			<div class="col">
				...
				<div class="pie-wrapper">
					<Pie shares={pie(langs.backend)} />
				</div>
			</div>
			<div class="col">
				<h2>Frontend</h2>
				<div class="pie-wrapper">
					<Pie shares={pie(langs.frontend)} />
				</div>
			</div>
		</div>
		<div class="row tag-wrapper">
			{#each allLangs as lang}
				<button
					class="tag {!$selectedTag || $selectedTag == lang ? '' : 'unselected'}"
					on:focus={() => focusTag(lang)}
					on:blur={() => blurTag(lang)}
					on:mouseover={() => focusTag(lang)}
					on:mouseleave={() => blurTag(lang)}
					on:click={() => selectTag(lang)}
					on:keydown={(e) => {
					...
					tabindex="0"
				>
					<span class="tag-c" style="--c: {colors[lang][0]}" />
					{lang}
				</button>
			{/each}
		</div>
	...
	<div class="col">
		...
		<div class="row contact-wrapper">
			<ContactIcon
				...
				content="shady@shadygoat.eu"
				link="mailto:shady@shadygoat.eu"
			/>
			<ContactIcon
				icon="fa-brands fa-linkedin"
				content="Lucy Dryaeva"
				link="https://www.linkedin.com/in/lucy-dryaeva"
			/>
			<ContactIcon icon="fa-brands fa-discord" content="@shadygoat" />
			<ContactIcon
				icon="fa-brands fa-github"
				content="ShadiestGoat"
				link="https://github.com/ShadiestGoat"
			/>
		</div>
	</div>
</section>
